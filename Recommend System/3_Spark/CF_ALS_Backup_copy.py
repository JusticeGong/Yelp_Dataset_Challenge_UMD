...
data = sc.textFile("file:/home/gz/Desktop/BigDataProject/Dataset/SubModelDataset00.txt")
...
#print('********The iteration = '+ str(numIterations) + ' and MAPE is ' + str(MAPE))
#MSE = ratesAndPreds.map(lambda r: (r[1][0] - r[1][1])**2).mean()
#print('########The iteration = '+ str(numIterations) + ' and MSE is ' + str(MSE))


for numIterations in range(11,16):
	rank = 10
	model = ALS.train(ratings, rank, numIterations)
	# Evaluate the model on training data
	testdata = ratings.map(lambda p: (p[0], p[1]))
	...
	MSE = ratesAndPreds.map(lambda r: (r[1][0] - r[1][1])**2).mean()
	print('########The interation = '+ str(numIterations) + ' and MSE is ' + str(MSE))


# Save and load model
#model.save(sc, "BigDataProject/Models/Model00")
